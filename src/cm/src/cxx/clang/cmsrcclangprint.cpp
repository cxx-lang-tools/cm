...
/// \file cmsrcclangprint.cpp
/// Contains code for the cmsrcclangprint utility for parsing and printint source code model.
...
int main(int argc, char * argv[]) {
    try {
        ...
        if (var_map.count("compare") != 0) {
            // comparing code model output with file
            std::ostringstream str;
            cm::src::ast_printer printer{str};
            printer.print(mdl);
            compare_string_with_file(str.str(), var_map["compare"].as<fs::path>());
        } else {
            // printing code model to stdout
            cm::src::ast_printer printer{std::cout};
            printer.print(mdl);
        }
    }
